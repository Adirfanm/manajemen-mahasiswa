<x-layouts::app :title="__('Tambah Mahasiswa')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">

        {{-- Header --}}
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-bold text-slate-900">Tambah Mahasiswa</h1>
                <p class="mt-1 text-sm text-slate-400">Isi data mahasiswa baru di bawah ini.</p>
            </div>
            <a href="{{ route('mahasiswa.index') }}" class="text-sm text-indigo-500 hover:text-indigo-400">← Kembali</a>
        </div>
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-6">
            ...
            <form method="POST" action="{{ route('mahasiswa.store') }}" class="flex flex-col gap-5">
                @csrf

                {{-- NIM --}}
                <div class="flex flex-col gap-1.5">
                    <label class="text-sm font-semibold text-slate-700">NIM</label>
                    <input type="text" name="nim" value="{{ old('nim') }}" placeholder="Contoh: 2312345678"
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <input type="text" name="jurusan" value="{{ old('jurusan') }}" placeholder="Contoh: Informatika"
                        class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400">
                </div>
